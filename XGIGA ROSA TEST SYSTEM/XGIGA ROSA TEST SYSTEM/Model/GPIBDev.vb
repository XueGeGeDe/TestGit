...
Public Class GPIBDev
    Private defaultRM As Integer = 0
    Private hInstr As Integer = 0
    Private Status As Integer = 0
    Private retCnt As Integer = 0
    Private isOpen As Boolean = False
    ''' <summary>
    ''' 通信失败标志位
    ''' </summary>
    Public Const FailFlag = -1024
    ...
    ''' <remarks></remarks>
    Public Function SendCommand(ByVal strCommand As String, Optional ByVal iTimeOut As Integer = 10) As Boolean
        Try
            Dim buffer() As Byte = Encoding.ASCII.GetBytes(strCommand)
            Status = viWrite(hInstr, buffer, buffer.Length, retCnt)
            Thread.Sleep(iTimeOut)
            If Status = VI_SUCCESS And retCnt <> 0 Then
                Return True
            Else
                Status = viWrite(hInstr, buffer, buffer.Length, retCnt)
                Thread.Sleep(iTimeOut)
        ...
        Catch ex As Exception
            MsgBox(DeviceName & ":" & ex.Message, vbCritical, "错误")
            CloseVISA()
            Return False
        End Try
    End Function
    ...
    ''' <returns>True:命令发送成功,False:命令发送失败</returns>
    ''' <remarks></remarks>
    Public Function SendCommand(ByVal arrCommand() As Byte, Optional ByVal iTimeOut As Integer = 20) As Boolean
        Try
            Dim buffer() As Byte = arrCommand
            Status = viWrite(hInstr, buffer, buffer.Length, retCnt)
            Thread.Sleep(iTimeOut)
            If Status = VI_SUCCESS And retCnt <> 0 Then
                Return True
    ...
    Public Sub ClearDataVISA()
        Status = viClear(hInstr)
    End Sub
    ...
    ''' <remarks></remarks>
    Public Function ReceiveData(ByRef strData As String, Optional ByVal Count As Integer = 0, Optional ByRef iCnt As Integer = 0) As Boolean
        Try
            If Count = 0 Then Count = 1024
            Dim buffer(Count - 1) As Byte
            Status = viRead(hInstr, buffer, Count, iCnt)
            If Status >= VI_SUCCESS And iCnt <> 0 Then
                strData = System.Text.Encoding.ASCII.GetString(buffer, 0, iCnt)
                Return True
            Else
                CloseVISA()
                Return False
    ...
    ''' <returns>True:接收数据成功,False:接收数据失败</returns>
    ''' <remarks></remarks>
    Public Function ReceiveData(ByRef binData() As Byte, Optional ByVal Count As Integer = 0, Optional ByRef iCnt As Integer = 0) As Boolean
        Try
            If Count = 0 Then Count = 1024
            Dim buffer(Count - 1) As Byte
            Status = viRead(hInstr, buffer, Count, iCnt)
            If Status >= VI_SUCCESS And iCnt <> 0 Then
                ReDim binData(iCnt - 1)
                Array.Copy(buffer, 0, binData, 0, iCnt)
                Return True
            Else
                CloseVISA()
                Return False
    ...
    ''' <remarks></remarks>
    Public Function SendAndReceiveData(ByVal arrCommand() As Byte, ByRef strData As String) As Boolean
        If SendCommand(arrCommand) = False Then Return False
        If ReceiveData(strData) = False Then Return False
        Return True
    End Function
    ...
    ''' <returns>True:发送和接收数据成功,False:发送或接收数据失败</returns>
    ''' <remarks></remarks>
    Public Function SendAndReceiveData(ByVal strCommand As String, ByRef strData As String) As Boolean
        If SendCommand(strCommand) = False Then Return False
        If ReceiveData(strData) = False Then Return False
        Return True
    End Function
    ...
    ''' <remarks></remarks>
    Public Sub OPC(Optional ByVal sTime As Integer = 1)
        Try
            Dim buffer() As Byte = Encoding.ASCII.GetBytes("*OPC?" + vbCrLf)
            Dim strValue As String = ""
            Dim nTimeOut As Integer
            Dim dwStartTime As Date = Now
            Dim DataBuff(255) As Byte
            Dim retVal As Integer
            Do
                Thread.Sleep(200)
                buffer = Encoding.ASCII.GetBytes("*OPC?" + vbCrLf)
                Status = viWrite(hInstr, buffer, buffer.Length, retCnt)
                Status = viRead(hInstr, DataBuff, 100, retCnt)
                If retCnt > 0 And retCnt < 255 Then
                    strValue = System.Text.Encoding.ASCII.GetString(DataBuff, 0, retCnt)
                End If
                If strValue.Contains("9.99999E") Then strValue = "0"
                retVal = Val(strValue)
                If retVal > 0 Then Exit Do
                nTimeOut = Date.Now.Subtract(dwStartTime).TotalSeconds
            Loop Until (nTimeOut >= (sTime * 1000))
        Catch ex As Exception
            MsgBox(DeviceName & ":OPC命令发送失败！", vbCritical, "错误")
            CloseVISA()
        End Try
    End Sub
    ''' <summary>
    ...
    ''' <remarks></remarks>
    Public Function OpenVISA(ByVal strAddress As String) As Boolean
        If isOpen = False Then
            Status = viOpenDefaultRM(defaultRM)
            Status = viOpen(defaultRM, strAddress, VI_NULL, VI_NULL, hInstr)
            Status = viSetAttribute(hInstr, VI_ATTR_TMO_VALUE, 5000)
            If Status = VI_SUCCESS Then
                isOpen = True
            Else
                MsgBox(DeviceName & ":GPIB通信失败，请检查连接或重启设备！", vbCritical, "错误")
                CloseVISA()
                Return False
            End If
        End If
    ...
    ''' <returns>True:设备通信成功，False:设备通信失败</returns>
    ''' <remarks></remarks>
    Public Function OpenVISA(ByVal strAddress As String, ByVal BoudRate As Integer) As Boolean
        If isOpen = False Then
            Status = viOpenDefaultRM(defaultRM)
            Status = viOpen(defaultRM, strAddress, VI_NULL, VI_NULL, hInstr)
            Status = viSetAttribute(hInstr, VI_ATTR_ASRL_BAUD, BoudRate)                   'rate
            Status = viSetAttribute(hInstr, VI_ATTR_ASRL_DATA_BITS, 8)                 'data bits
            Status = viSetAttribute(hInstr, VI_ATTR_ASRL_PARITY, VI_ASRL_PAR_NONE)     'par bit
            Status = viSetAttribute(hInstr, VI_ATTR_ASRL_STOP_BITS, VI_ASRL_STOP_ONE)  'stop bit
            Status = viSetAttribute(hInstr, VI_ATTR_TMO_VALUE, 5000)                   'timeout
            If Status = VI_SUCCESS Then
                isOpen = True
            Else
                MsgBox(DeviceName & ":GPIB通信失败，请检查连接或重启设备！", vbCritical, "错误")
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Public Sub CloseVISA()
        isOpen = False
        Status = viClose(hInstr)
    ...
    Public Function SearchKeyData(ByVal KeyValue As String, ByVal KeyData As List(Of String)) As String
        Try
            If KeyData.Count <= 0 Then Return ""
            Dim arrKeys() As String = Split(KeyValue, ",")
            For j As Integer = 0 To arrKeys.Length - 1
                For i = 0 To KeyData.Count - 1
                    If InStr(KeyData(i).ToUpper, arrKeys(j).ToUpper) Then
                        Return KeyData(i).Substring(InStr(KeyData(i), ":"))
                    End If
                Next
            Next
            Return ""
    ...
    Public Sub unLockVISA()
        If hInstr > 0 Then Status = viUnlock(hInstr)
    End Sub

    Public Function LockVISA() As Boolean
        Dim i As Integer
        Dim STATE As Byte
        Dim mDeviceString As New System.Text.StringBuilder(255)
        i = 0
        Do
            Status = viGetAttribute(hInstr, VI_ATTR_RSRC_LOCK_STATE, STATE)
            If STATE = VI_EXCLUSIVE_LOCK Then Exit Do
            Status = viLock(hInstr, VI_EXCLUSIVE_LOCK, 1000, "", mDeviceString)
            i += 1
        Loop Until (Status = VI_SUCCESS Or i > 30)
        Return IIf(Status = VI_SUCCESS, True, False)
    End Function
End Class
